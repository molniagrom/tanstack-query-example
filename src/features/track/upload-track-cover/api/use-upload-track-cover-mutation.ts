...
export const useUploadTrackCoverMutation = () => {
    ...
    const uploadWithXHR = (trackId: string, file: File, onProgress?: (progress: number) => void): Promise<unknown> => {
        return new Promise((resolve, reject) => {
            ...
            xhr.open('POST', `${baseUrl}playlists/tracks/${trackId}/cover`)

            const accessToken = localStorage.getItem(localStorageKeys.accessToken)
            if (accessToken) xhr.setRequestHeader('Authorization', `Bearer ${accessToken}`)
            if (apiKey) xhr.setRequestHeader('api-key', apiKey)
    ...
    return useMutation({
        ...
        mutationFn: async ({trackId, file, onProgress}: UploadCoverParams) => {
            try {
                return await uploadWithXHR(trackId, file, onProgress)
            } catch (error) {
